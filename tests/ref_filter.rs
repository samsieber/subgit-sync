...
pub fn import_good_ref_but_not_bad_refs() {
    let test = TestWrapper::new("import_good_ref_but_not_bad_ref", |upstream| {
        ...
        upstream.push_adv(vec!("origin", &format!("HEAD:{}", "refs/test/bad"))).unwrap();

        upstream.command_output(vec!("tag","-a","tag_heavy", "-m", "my heavy tag")).unwrap();
    ...
    assert_eq!(test.get_subgit().command_output(vec!("show-ref", "bad")).unwrap().trim(), "");
    assert_eq!(test.get_subgit().command_output(vec!("show-ref", "tag_heavy")).unwrap().trim(), "");
    assert_eq!(test.get_subgit().command_output(vec!("show-ref", "tag_light")).unwrap().trim(), "");
}

#[test]
pub fn upstream_push_good_ref_but_not_bad_refs() {
    let test = TestWrapper::new("upstream_push_good_ref_but_not_bad_refs", |upstream| {
        upstream.update_working(vec![
            FileAction::overwrite("sub/hello.txt", "Hello world (from upstream)"),
        ]);
        upstream.add(".").unwrap();
...
pub fn downstream_push_good_ref_but_not_bad_refs() {
    let test = TestWrapper::new("downstream_push_good_ref_but_not_bad_refs", |upstream| {
        upstream.update_working(vec![
            FileAction::overwrite("sub/hello.txt", "Hello world (from upstream)"),
        ]);
        upstream.add(".").unwrap();
    ...
    test.do_then_verify(|_upstream, downstream| {
        ...
        downstream.push_adv(vec!("origin", &format!("HEAD:{}", "refs/test/bad"))).unwrap();

        downstream.command_output(vec!("tag","-a","tag_heavy", "-m", "my heavy tag")).unwrap();
    ...
    assert!(test.get_subgit().command_output(vec!("show-ref", "master")).unwrap().trim().contains("master"));
    assert_eq!(test.get_upstream().command_output(vec!("show-ref", "bad")).unwrap().trim(), "");
    assert_eq!(test.get_upstream().command_output(vec!("show-ref", "tag_heavy")).unwrap().trim(), "");
    assert_eq!(test.get_upstream().command_output(vec!("show-ref", "tag_light")).unwrap().trim(), "");
}