...
pub fn write_content_to_file<P: AsRef<Path>, S: AsRef<str>>(path : &P, content: &S){
    ...
    file.write_all(content.as_ref().as_bytes()).expect("Unable to write data");
}
...
pub fn symlink_dirs<SP: AsRef<Path>, DP: AsRef<Path>>(source: &SP, dest: &DP, dirs: &Vec<&str>) -> Result<(), Box<Error>>{
    let abs_source = make_absolute(&source)?;
    let links : Result<Vec<()>, _> = dirs.iter()
        .map(|&dir| -> Result<(),Box<Error>> {
            Ok(std::os::unix::fs::symlink(abs_source.join(dir), dest.as_ref().join(dir))?)
        }).collect();
    links?;
    Ok(())
}